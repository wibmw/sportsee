  ...
  const DrawChart = ({ session }: IActivitySessions) => {
    ...
    const lines = (isWeight: boolean, isRounded: boolean, className: string) => {
      ...
      const line = svg
        ...
        .enter()
        .append('line')
        .attr('x1', (d, index) => xScale(new Date(d.day).getDate()) + xDiff + (index === 0 ? 10 : 0) + (index === 6 ? -10 : 0))
        .attr('x2', (d, index) => xScale(new Date(d.day).getDate()) + xDiff + (index === 0 ? 10 : 0) + (index === 6 ? -10 : 0))
